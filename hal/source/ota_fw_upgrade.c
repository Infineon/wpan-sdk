...
#define DECOMPRESS_BUF_SIZE FLASH_NOR_PAGE_SIZE
//==================================================================================================
// Types
//==================================================================================================
...
static bool src_data_reader(const void * src, size_t n, uint8_t * compressed_buf, uint32_t read_byte_count)
 ...
 ******************************************************/
__attribute__((section(".text_in_ram"))) void wiced_firmware_upgrade_bootloader(void)
{
    ...
    uint32_t compressed_data_addr   = (uint32_t) upgrade_xs->compressed_data;

#if BOARD_USE_EXTERNAL_FLASH
    spi_pin_config();

    wiced_hal_pspi_reset(FLASH_NOR_SPI_INTERFACE);
    wiced_hal_pspi_init(FLASH_NOR_SPI_INTERFACE, FLASH_NOR_SPI_CLOCK_RATE, SPI_MSB_FIRST, SPI_SS_ACTIVE_LOW, SPI_MODE_0);

    if (cy_serial_flash_init() != FLASH_NOR_RET_SUCCESS)
    {
        goto exit;
    }
    uint8_t xs_info[sizeof(upgrade_xs_t)]; // [2]: compressed_data_crc32, [3]: compressed_data_length
    cy_serial_flash_read(EXT_FLASH_XS_START_ADDR, (uint8_t *) xs_info, sizeof(xs_info));
    upgrade_xs           = (upgrade_xs_t *) xs_info;
    compressed_data_addr = offsetof(upgrade_xs_t, compressed_data);
#endif
    ...
    if (!lzss_decompress((const void *) compressed_data_addr, upgrade_xs->compressed_data_length, src_data_reader,
                         dest_data_writer))
    {
        goto reset;
    }
...
#if BOARD_USE_EXTERNAL_FLASH
    cy_serial_flash_erase(EXT_FLASH_XS_START_ADDR, FLASH_NOR_ERASE_SECTOR);
#else
...
bool wiced_firmware_upgrade_prepare(void)
{
    ...
    const uint32_t upgrade_xs_length = EXT_FLASH_XS_END_ADDR - EXT_FLASH_XS_START_ADDR;
    printf("Erasing external flash: 0x%0x, len: 0x%08lx\n", EXT_FLASH_XS_START_ADDR, upgrade_xs_length);
    uint32_t target_addr = EXT_FLASH_XS_START_ADDR;
    do
    {
        cy_serial_flash_erase(target_addr, FLASH_NOR_ERASE_SECTOR);
        target_addr += FLASH_NOR_SECTOR_SIZE;
    } while ((target_addr & 0xfffff000) <= (EXT_FLASH_XS_END_ADDR & 0xfffff000));
...
uint32_t wiced_firmware_upgrade_process_block(uint32_t offset, const uint8_t * data, uint32_t len)
{
    ...
    {
#if BOARD_USE_EXTERNAL_FLASH
        byte_written += upgrade_location_ext_flash_write(ext_flash_upgrade_address, data + ds_write_length, xs_write_length);
        ext_flash_upgrade_address += xs_write_length;
#else
...
uint32_t upgrade_location_ext_flash_write(uint32_t offset, const uint8_t * data, uint32_t len)
{
    ...
    /* write length should in words */
    if (cy_serial_flash_write(offset, (uint8_t *) data, len) == FLASH_NOR_RET_SUCCESS)
    {
        return len;
    }
    else
...
bool wiced_firmware_upgrade_finalize(void)
{
...
#if BOARD_USE_EXTERNAL_FLASH
    uint8_t crc_buffer[FLASH_NOR_PAGE_SIZE] = { 0 };
    uint8_t xs_info[sizeof(upgrade_xs_t)]; // [2]: compressed_data_crc32, [3]: compressed_data_length
    if (cy_serial_flash_read(EXT_FLASH_XS_START_ADDR, (uint8_t *) xs_info, sizeof(xs_info)) != FLASH_NOR_RET_SUCCESS)
    {
        return false;
    }
    upgrade_xs = (upgrade_xs_t *) xs_info;
    ...
    uint32_t i                      = 0;
    uint32_t cx_crc32               = 0xffffffff;
    uint32_t ext_flash_cx_data_addr = 0;
    uint32_t total_crc_buffer_count = upgrade_xs->compressed_data_length / sizeof(crc_buffer);
    uint32_t remain_crc_buffer_size = upgrade_xs->compressed_data_length % sizeof(crc_buffer);
    uint32_t crc32_Update(uint32_t crc, const uint8_t * buf, uint16_t len);

    for (i = 0; i < total_crc_buffer_count; i++)
    {
        ext_flash_cx_data_addr = offsetof(upgrade_xs_t, compressed_data) + i * sizeof(crc_buffer);
        if (cy_serial_flash_read(ext_flash_cx_data_addr, crc_buffer, sizeof(crc_buffer)) != FLASH_NOR_RET_SUCCESS)
        {
            return false;
        }
        cx_crc32 = crc32_Update(cx_crc32, (uint8_t *) crc_buffer, sizeof(crc_buffer));
    }
    if (cy_serial_flash_read(ext_flash_cx_data_addr + sizeof(crc_buffer), crc_buffer, remain_crc_buffer_size) !=
        FLASH_NOR_RET_SUCCESS)
    {
        return false;
    }
    cx_crc32 = crc32_Update(cx_crc32, (uint8_t *) crc_buffer, remain_crc_buffer_size);
    cx_crc32 ^= 0xffffffff;
#else
                     ...
                     bool (*data_writer)(uint32_t, int))
{
    ...
    int r;
    int c; // one data byte read from compressed buffer

    unsigned int flags;
    uint32_t dc_offset       = 0;
    uint32_t read_byte_count = 0;

    uint8_t compressed_buf[DECOMPRESS_BUF_SIZE];
    const uint8_t * s = compressed_buf; // s means current compressed buffer address
    ...
    while (read_byte_count < n)
    {
        if (((flags >>= 1) & 0x100) == 0)
        {
            ...
            if (read_byte_count % DECOMPRESS_BUF_SIZE == 0)
            {
                if (!data_reader(src, n, compressed_buf, read_byte_count))
                {
                    return FALSE;
                }
                s = compressed_buf;
...
bool src_data_reader(const void * src, size_t n, uint8_t * compressed_buf, uint32_t read_byte_count)
{
    ...
    uint32_t remain_ext_flash = n - read_byte_count;
    uint32_t read_size        = (remain_ext_flash >= DECOMPRESS_BUF_SIZE) ? DECOMPRESS_BUF_SIZE : (remain_ext_flash);
...
bool dest_data_writer(uint32_t data_offset, int c)
{
...
void spi_pin_config(void)
{
    ...
    wiced_hal_gpio_select_function(PLATFORM_SPI_2_MISO, WICED_SPI_2_MISO);
    wiced_hal_gpio_select_function(PLATFORM_SPI_2_MOSI, WICED_SPI_2_MOSI);
    wiced_hal_gpio_select_function(PLATFORM_SPI_2_SCLK, WICED_SPI_2_CLK);
}

void spi_pin_reset(void)
{
    wiced_hal_gpio_select_function(PLATFORM_SPI_2_CS, WICED_GPIO);
    wiced_hal_gpio_select_function(PLATFORM_SPI_2_MISO, WICED_GPIO);
