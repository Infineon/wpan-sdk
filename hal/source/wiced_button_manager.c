 ...
 ******************************************************/
static void button_state_change_callback(button_manager_button_t * button, bool is_pressed);
static wiced_result_t button_pressed_event_handler(void * arg);
static wiced_result_t button_released_event_handler(void * arg);
static void button_gpio_interrupt_handler(void * user_data, uint8_t pin);
static bool button_is_pressed(const button_manager_button_t * button);
...
static const wiced_platform_button_config_t * button_platform_config_get(wiced_bt_gpio_numbers_t gpio);
#ifdef CYW55572
...
static void button_long_press_detect_timeout_handler(TIMER_PARAM_TYPE arg)
{
    ...
    p_button->current_state = button_is_pressed(p_button) ? BUTTON_STATE_HELD : BUTTON_STATE_RELEASED;

    if (p_button->current_state != BUTTON_STATE_HELD)
    {
        WICED_BT_TRACE("Err: Button %d is already released\n", p_button->configuration->gpio);
...
static void button_debounce_timeout_handler(TIMER_PARAM_TYPE arg)
{
    ...
    // WICED_BT_TRACE("button_debounce_timeout_handler (%d, %d)\n", p_button->configuration->gpio, p_button->debounce_counter);
                          ...
                          button_manager_button_t * buttons, uint32_t number_of_buttons)
{
    ...
    {
        buttons[a].platform = button_platform_config_get(buttons[a].configuration->gpio);
        if (buttons[a].platform == NULL)
        {
            WICED_BT_TRACE("Err: No button config for pin %d\n", buttons[a].configuration->gpio);
            continue;
        }

        wiced_hal_gpio_register_pin_for_interrupt(*buttons[a].platform->gpio, button_gpio_interrupt_handler, &buttons[a]);
...
void button_gpio_interrupt_handler(void * user_data, uint8_t pin)
{
    button_manager_button_t * button = user_data;
    if (button == NULL)
    {
        return;
    }

    button_state_change_callback(button, button_is_pressed(button));
}

bool button_is_pressed(const button_manager_button_t * button)
{
    return wiced_hal_gpio_get_pin_input_status(*button->platform->gpio) == button->platform->button_pressed_value;
}
 ...
 */
void button_state_change_callback(button_manager_button_t * button, bool is_pressed)
{
#if 0
...
const wiced_platform_button_config_t * button_platform_config_get(wiced_bt_gpio_numbers_t gpio)
{
    uint8_t a;

    for (a = 0; a < button_count; a++)
    {
        if (*platform_button[a].gpio == gpio)
        {
            return &platform_button[a];
        }
    }
...
void wiced_button_manager_pending_event_handle(void)
{
    ...
    if (wiced_sleep_get_boot_mode() == WICED_SLEEP_FAST_BOOT)
    {
        ...
        {
            if (wiced_hal_platform_gpio_int_status_get_and_clear(*button_manager->buttons[a].platform->gpio) == 1)
            {
